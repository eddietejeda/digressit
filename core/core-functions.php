...
function activate_digressit(){
    ...
    $is_multiuser = digressit_is_multisite();            
	...
	/*
	 * @TODO these are a bunch of variables that were used or was planning to use. I think we should trim this this!
	 */
 ...
 */
function digressit_is_multisite(){
    $is_multiuser = false;
...
//    var_dump($pieces);
    ...
    elseif(digressit_is_multisite() && $blog_id == 1 && digressit_is_frontpage()){
...
function digressit_core_print_styles(){
    ...
    if($blog_id == 1 && file_exists(ABSPATH.'/wp-content/plugins/buddypress/bp-core/bp-core-cssjs.php') && function_exists('bp_core_add_admin_bar_css')){
        ...
        if ( digressit_is_multisite()  || is_admin() ) {
...
function get_root_domain(){
    ...
    $production_mode = false;
     ...
     return digressit_is_multisite() ? network_site_url() : site_url();
...
function digressit_core_print_scripts(){
    ...
    if (!is_admin() && $digressit_options['debug_mode'] == 1) {
        wp_register_script('html5.js', get_digressit_media_uri('js/jquery.js'));
    }
	elseif (!is_admin()){
	    wp_register_script('html5.js', 'http://html5shim.googlecode.com/svn/trunk/html5.js', 'jquery', false, false );
	}
...
var is_multisite = <?php echo digressit_is_multisite() ? 1 : 0; ?>;
var siteurlpath = '<?php echo digressit_is_multisite() ? network_site_url() : site_url(); ?>';
